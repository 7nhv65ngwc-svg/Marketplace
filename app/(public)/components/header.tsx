...
function SidebarItem({ href, label }: { href: string, label: string }) {
    return (
        <Link href={href}>
            <li className="flex flex-row items-center gap-2 justify-between hover:bg-gray-50 hover:text-[#00BC99] curso-pointer px-2 py-1.5 border-b border-slate-200">
                {label}
                <FaAngleRight />
            </li>
        </Link>
        ...
        <>
            ...
            <section className={`absolute h-svh bg-white shadow-lg rounded-tr-md rounded-br-md transition-all duration-700 ease-in-out ${show ? "w-50" : "w-0"}`}>
                <div className="relative h-full">
                    {show && (<button type="button" onClick={() => setShow(false)} className="absolute -right-3 top-2 bg-[#00BC99] flex items-center justify-center text-white h-6 w-6 rounded-md cursor-pointer hover:text-[#012E40] shadow-lg">
                        <IoClose />
                    </button>)}
            ...
            <section className={`absolute inset-y-0 right-0 bg-white shadow-lg rounded-tr-md rounded-br-md transition-all duration-700 ease-in-out ${showcart ? "w-50" : "w-0"}`}>
                <div className="relative h-full">
                    {setShowCart && (<button type="button" onClick={() => setShowCart(false)} className="absolute -left-3 top-2 bg-[#00BC99] flex items-center justify-center text-white h-6 w-6 rounded-md cursor-pointer hover:text-[#012E40] shadow-lg">
                        <IoClose />
                    </button>)}
